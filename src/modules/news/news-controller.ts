...
import * as parser from "./news-parser";
import * as _ from "lodash";

const newsParser = parser.default;
const newsApi = api.default;
...
function getTemplate(key: string, id: any, req: Request) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (_htmlStr) {
        const _isMemCached = key === "news" ? JSON.parse(_htmlStr) : _htmlStr;
        resolve(_isMemCached);
      } else {
        redisCache.default.getCache(_key + _devicePrefix, function(redisResp: any) {
          if (redisResp) {
            const _isRedisCached = key === "news" ? JSON.parse(redisResp) : redisResp;
            customMemcache.default.setCache(_key + _devicePrefix, _isRedisCached);
            resolve(_isRedisCached);
          } else {
            return newsApi.requestApi(key, id).then(function(response: any) {
              const _htmlStr = newsParser.htmlToString(key, response, req);
              const _isObj = key === "news" ? JSON.stringify(_htmlStr) : _htmlStr;
    ...
    if (newsId) {
      ...
      .then(function(results: any) {
          ...
          const obj = {
              ...
              storyContentTemplate: results[1].newsContent,
