...
function htmlToString(partialName: string, response: any, req: Request) {
    ...
    if (_keyName === "schedule-widget") {
      ...
      return ejs.render(fs.readFileSync(_viewPath, "utf8"), {[partialName]: _data});
        ...
        let _obj = {
            "newsContent": ejs.render(fs.readFileSync(_viewPath, "utf8"), _data)
        };
...
function getCss() {
    return  {
        style : uglifycss.processString(fs.readFileSync(serverPathHelper.default.get().serverPath + "/public/css/news-desktop.css", "utf8"))
    };
}

function getMobileCss() {
    return  {
        style : uglifycss.processString(fs.readFileSync(serverPathHelper.default.get().serverPath + "/public/css/news-mobile.css", "utf8"))
    };
}
...
function getMobileStoriesPage(pageData: any) {
    ...
    const _viewPath = path.join(serverPathHelper.default.get().serverPath, serverPathHelper.default.get().viewsPath + "/news-mobile.ejs");
...
function getStoriesPage(pageData: any) {
    ...
    const _viewPath = path.join(serverPathHelper.default.get().serverPath, serverPathHelper.default.get().viewsPath + "/news-desktop.ejs");
    return minify(ejs.render(fs.readFileSync(_viewPath, "utf8"), pageData), minifyConfig);
}
